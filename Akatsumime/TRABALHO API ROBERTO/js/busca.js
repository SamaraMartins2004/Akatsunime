document.addEventListener("DOMContentLoaded", () => {
  ...
  const bntBuscar = document.querySelector("#bntBuscar");
  ...
  const urlAnimesBemAvaliados =
    "https://kitsu.io/api/edge/anime?sort=-averageRating&page[limit]=21";
  const urlAnimesRecentes =
    "https://kitsu.io/api/edge/anime?sort=-startDate&page[limit]=20";
  ...
  bntBuscar.addEventListener("click", () => {
    const procurar = inputBusca.value.trim();

    if (procurar.length > 0) {
      const urlBusca = `https://kitsu.io/api/edge/anime?filter[text]=${procurar}&page[limit]=10`;
      buscarAnimes(urlBusca, gridAnimesPopulares);
    } else {
      buscarAnimes(urlAnimesPopulares, gridAnimesPopulares);
    }
  });
